...
class Race extends EventEmitter {
  ...
  start() {
    ...
    var delay = this._config.race.countdown.delay;
    var promise = bluebird.resolve();
    for (var i = this._config.race.countdown.counts; i > 0; i--) {
      promise = this._updatePromise(promise, i, delay);
    }
  ...
  _updatePromise(promise, i, delay) {
    return promise.then(() => {
      this.emit('countdown', { count: i });
