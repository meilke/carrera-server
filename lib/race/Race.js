...
class Race extends EventEmitter {
  ...
  start(newPlayers) {
    ...
    return this._countdown
      ...
      .then(() => {
        this._startTime = Date.now();
        this._players = _.map(newPlayers, _.partial(this._newPlayer, this._startTime));
  ...
  _updatePlayer(player) {
    ...
    if (player.signals > 0 && !this._lapOfSufficientTime(player, newTime)) {
      return;
    }
  ...
  _lapOfSufficientTime(player, time) {
    return (time - player.lastTime) > this._config.signal.ignoreUnderMs;
  }
